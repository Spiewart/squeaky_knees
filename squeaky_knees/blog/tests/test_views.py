...
class TestEmptyBlogIndex:
    ...
    def test_blog_index_with_posts_hides_empty_message(self, blog_index, blog_post, client):
        ...
        # The empty message might still be in template but shouldn't be visible
        # because blogpages will have items
    ...
    def test_blog_index_shows_pagination_controls(self, blog_index, client, user):
        ...
        # Create 12 posts to force pagination (default is 10 per page)
        for i in range(12):
            post = BlogPage(
                title=f"Post {i}",
                date=date.today(),
    ...
    def test_blog_index_pagination_page_two(self, blog_index, client, user):
        ...
        assert response.status_code == 200
        # Page 2 should exist and render without error
        content = response.content.decode()
